...
import {config,initialCards,profileName,profileJob,nameInput,jobInput,cardFormElement,profileFormElement,cardsContainer,addBtn,editBtn,popupAddCard,popupCardImg,popupEditProfile} from "../scripts/utils//constant.js"
import './index.css'; 
...
const popupAddCardElement = new PopupWithForm({popup : popupAddCard, handleFormSubmit: (evt,item) => {
    evt.preventDefault();
    console.log(item)
    const addCard = new Card({data: {name:item.cardNameInput,link: item.cardLinkInput }, handleCardClick: () => {
        const imagePopupClass = new PopupWithImage(popupCardImg);
        imagePopupClass.setEventListeners();
...
const popupEditProfileElement = new PopupWithForm({popup : popupEditProfile, handleFormSubmit: (evt,info) => {
   evt.preventDefault();
